...
namespace SpriteSheetPipeline
{
    ...
    public static class SpritePacker
    {
        ...
        /// </summary>
        static BitmapContent CopySpritesToOutput(List<ArrangedSprite> aSprites,
                                                 IList<BitmapContent> aSourceSprites,
                                                 ICollection<Rectangle> aOutputSprites,
                                                 int width, int height)
        {
            ...
            foreach (ArrangedSprite sprite in aSprites)
            {
                BitmapContent source = aSourceSprites[sprite.Index];
                ...
                aOutputSprites.Add(new Rectangle(x + 1, y + 1, w, h));
            }
            return output;
        }
        ...
        /// </summary>
        static void PositionSprite(List<ArrangedSprite> aSprites,
                                   int aIndex, int aOutputWidth)
        {
            int x = 0;
            int y = 0;
            ...
            while (true)
            {
                ...
                int intersects = FindIntersectingSprite(aSprites, aIndex, x, y);
                ...
                x = aSprites[intersects].X + aSprites[intersects].Width;

                // If we ran out of room to move to the right,
                // try the next line down instead.
                if (x + aSprites[aIndex].Width > aOutputWidth)
                {
                    x = 0;
                    y++;
                }
        ...
        /// </summary>
        static int FindIntersectingSprite(List<ArrangedSprite> aSprites,
                                          int aIndex, int aX, int aY)
        {
            int w = aSprites[aIndex].Width;
            int h = aSprites[aIndex].Height;

            for (int i = 0; i < aIndex; i++)
            {
                if (aSprites[i].X >= aX + w)
                    continue;

                if (aSprites[i].X + aSprites[i].Width <= aX)
                    continue;

                if (aSprites[i].Y >= aY + h)
                    continue;

                if (aSprites[i].Y + aSprites[i].Height <= aY)
                    continue;
        ...
        /// </summary>
        static int GuessOutputWidth(List<ArrangedSprite> aSprites)
        {
            ...
            int width = medianWidth * (int)Math.Round(Math.Sqrt(aSprites.Count));
