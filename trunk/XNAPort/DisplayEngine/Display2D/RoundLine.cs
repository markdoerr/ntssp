...
namespace DisplayEngine.Display2D
{
    ...
    public class RoundLineManager
    {
        ...
        public void Init(GraphicsDevice device, ContentManager content)
        {
            ...
            {
                // Wide window
                projScaleX = (height / width) / width;
                projScaleY = (1.0f) / height;
            }
            else
            {
                // Tall window
                projScaleX = (1.0f) / width;
                projScaleY = (width / height) / height;
            }
            mProjection = Matrix.CreateScale(projScaleX, projScaleY, 0.0f);
            mProjection.M43 = 0.5f;
