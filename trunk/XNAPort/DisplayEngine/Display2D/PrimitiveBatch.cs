...
namespace DisplayEngine.Display2D
{
    ...
    public class PrimitiveBatch : IDisposable
    {
        ...
        RoundLineManager mRoundLineManager;
        ...
        public PrimitiveBatch(GraphicsDevice graphicsDevice, Game aGame)
        {
            ...
            mRoundLineManager.Init(device, aGame.Content);
        ...
        private void Flush()
        {
            ...
            if (primitiveType == ePrimitiveType.RoundedLineList)
            {
                ...
                //lines.Add(new RoundLine(new Vector2(0.0f),new Vector2(0.5f)));
                for (int i = 0; i < primitiveCount * 2; i++)
                {
                    if(i % 2 != 0)
                    {
                        lines.Add(new RoundLine(vertices[i-1].Position.X,vertices[i-1].Position.Y,vertices[i].Position.X,vertices[i].Position.Y));
                    }
                }

                mRoundLineManager.Draw(lines, LineRadius, Color.Brown, 2000, null);
            }
            // submit the draw call to the graphics card
            else
            {

                PrimitiveType type = PrimitiveType.PointList;
                ...
                device.DrawUserPrimitives<VertexPositionColor>(type, vertices, 0,
                    primitiveCount);
            }
